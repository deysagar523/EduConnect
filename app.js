...
const adminloginCallback = async (adminname, adminpassword, cb) => {
  try {
    ...
    bcrypt.compare(adminpassword, admin.password, function (err, result) {
      // console.log(result);
      // console.log(user.password);
      if (result === true) return cb(null, admin);
      else {
        return cb(null, false, {
          error: "Incorrect password",
          invalid: {
  ...
  new GoogleStrategy(
    {
      ...
      callbackURL:
        "https://blooming-reaches-75363.herokuapp.com/auth/google/secrets",
...
app.post("/notice", function (req, res) {
  ...
  notice.save();
  if (req.cookies.adminusername) res.redirect("/Deletenotice");
  else res.redirect("/admin_signin");
});
            ...
            function (err, result) {
              ...
              if (result === true) {
                if (
                  ...
                  req.body.cnewPassword.length < 3
                ) {
              ...
              },
              {
                ...
                studentDetails: {
                  ...
                  fatherDetails: {
                    ...
                    fathername:
                      founduser.studentDetails.fatherDetails.fathername,
                    occupation:
                      founduser.studentDetails.fatherDetails.occupation,
                    mobileno:
                      fatherMobile.length > 0
                        ? fatherMobile
                        : founduser.studentDetails.fatherDetails.mobileno,
                    email:
                      fatherEmail.length > 0
                        ? fatherEmail
                        : founduser.studentDetails.fatherDetails.email,
                  },
                  motherDetails: {
                    _id: founduser.studentDetails.motherDetails._id,
                    mothername:
                      founduser.studentDetails.motherDetails.mothername,
                    occupation:
                      founduser.studentDetails.motherDetails.occupation,
                    motherMobileno:
                      motherMobile.length > 0
                        ? motherMobile
                        : founduser.studentDetails.motherDetails.motherMobileno,
                    motherEmail:
                      motherEmail.length > 0
                        ? motherEmail
                        : founduser.studentDetails.motherDetails.motherEmail,
                  },
                },
              },
              function (err) {
      ...
      if (err) {
        ...
        students.forEach(function (student) {
          if (student.studentDetails) {
            if (student.studentDetails.class === key) s.push(student);
          }
        });
        s.sort(compare);
        // console.log(s);
